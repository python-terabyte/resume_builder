    ...
    <div className="grid grid-cols-3 text-gray-900" style={{ minHeight: '297mm' }}>
      ...
      >
        ...
        {isSectionVisible(resume, 'skills') && resume.skillGroups.length > 0 && (
          <SideHeader>{sectionTitle(resume, 'skills')}</SideHeader>
        )}
        {isSectionVisible(resume, 'skills') && resume.skillGroups.map((g) => (
        ...
        {isSectionVisible(resume, 'languages') && resume.languages.length > 0 && (
          <>
            <SideHeader>{sectionTitle(resume, 'languages')}</SideHeader>
        ...
        {isSectionVisible(resume, 'interests') && resume.interests.length > 0 && (
          <>
            <SideHeader>{sectionTitle(resume, 'interests')}</SideHeader>
      ...
      <main className="col-span-2 px-8 py-8 space-y-5">
        {personal.summary && (
          <MainBlock title="About" accentColor={accentColor}>
        ...
        {isSectionVisible(resume, 'experience') && resume.experience.length > 0 && (
          <MainBlock title={sectionTitle(resume, 'experience')} accentColor={accentColor}>
        ...
        {isSectionVisible(resume, 'projects') && resume.projects.length > 0 && (
          <MainBlock title={sectionTitle(resume, 'projects')} accentColor={accentColor}>
        ...
        {isSectionVisible(resume, 'education') && resume.education.length > 0 && (
          <MainBlock title={sectionTitle(resume, 'education')} accentColor={accentColor}>
            {resume.education.map((edu) => (
              <div key={edu.id} className="mb-2">
                ...
                <div className="text-sm text-gray-700">{[edu.degree, edu.field].filter(Boolean).join(', ')}{edu.gpa && <span className="text-gray-500"> · GPA: {edu.gpa}</span>}</div>
              </div>
            ))}
          </MainBlock>
        )}
        ...
        {isSectionVisible(resume, 'certifications') && resume.certifications.length > 0 && (
          <MainBlock title={sectionTitle(resume, 'certifications')} accentColor={accentColor}>
