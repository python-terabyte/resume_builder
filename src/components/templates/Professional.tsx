      ...
      <div className="mt-5 grid grid-cols-3 gap-6">
        <aside className="col-span-1 space-y-4 text-xs">
          ...
          {isSectionVisible(resume, 'skills') && resume.skillGroups.length > 0 && (
            <SideBlock title={sectionTitle(resume, 'skills')} accentColor={accentColor}>
          ...
          {isSectionVisible(resume, 'languages') && resume.languages.length > 0 && (
            <SideBlock title={sectionTitle(resume, 'languages')} accentColor={accentColor}>
              {resume.languages.map((l) => (
                <div key={l.id}><span className="font-semibold">{l.language}</span>{l.proficiency && <span className="text-gray-600"> · {l.proficiency}</span>}</div>
              ))}
            </SideBlock>
          )}

          {isSectionVisible(resume, 'certifications') && resume.certifications.length > 0 && (
            <SideBlock title={sectionTitle(resume, 'certifications')} accentColor={accentColor}>
          ...
          {isSectionVisible(resume, 'interests') && resume.interests.length > 0 && (
            <SideBlock title={sectionTitle(resume, 'interests')} accentColor={accentColor}>
        ...
        <main className="col-span-2 space-y-5">
          {personal.summary && (
            <MainBlock title="Profile" accentColor={accentColor}>
